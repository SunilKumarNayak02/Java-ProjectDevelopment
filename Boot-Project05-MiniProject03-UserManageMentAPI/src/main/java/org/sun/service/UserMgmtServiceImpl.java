...
public class UserMgmtServiceImpl implements IUserMgmtService {
    ...
    public String registerUser(UserAccount user) throws Exception {
        ...
        //Return the Mesage
        return savedMaster.getUserId()!= null ? "User is Registerd With Id Value Check Your Mail-ID: " + savedMaster.getUserId() : "Problem is user Registration";
    }

   /* @Override
    public String activateUser(ActivateUser user) {
        //Convert   ActiveUser obj to Entity obj (UserMaster)data
        UserMaster master = new UserMaster();
        //properties are not matching better to go for the manual copy
        master.setEmail(user.getEmail());
        master.setPassword(user.getTempPassword());
        //check the record availability by using  email and temp Password
        ...
        //if the valid  email and temp password id given then set enduser supplyed Real Password to Update the Record
        if (list.size() != 0) {
            //get Entity Object
    ...
    //Anather Way we can developed go to repository type one method
   @Override
   public String activateUser(ActivateUser user) {
      //Use findBy Method
